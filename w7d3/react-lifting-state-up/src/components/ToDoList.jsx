...
function ToDoList() {
  ...
  const toggleTaskDone = (id) => {
    const tasksCopy = [...tasks]; // copy of the state variable "tasks"
      
      tasksCopy.forEach((task) => {
      // Find the selected task and update task's `isDone` property,
      if (task._id === id) {
        task.isDone = !task.isDone;  // task.isDone = task.isDone ? false : true;
        ...
        const updateTasksCompleted =  task.isDone ? tasksCompleted + 1 : tasksCompleted - 1;
        setTasksCompleted(updateTasksCompleted);
      }
